import { stringStorage, StorageKeys } from '../utils/storage';

const DAILY_REWARD_KEY = StorageKeys.DAILY_REWARD_LAST_DATE;
...
export const hasClaimedDailyRewardToday = async (): Promise<boolean> => {
  try {
    const lastDate = await stringStorage.get(DAILY_REWARD_KEY);
    if (!lastDate) {
      return false;
    }
...
export const claimDailyReward = async (): Promise<void> => {
  try {
    ...
    await stringStorage.set(DAILY_REWARD_KEY, today);
  } catch (error) {
    if (__DEV__) {
...
export const getLastRewardDate = async (): Promise<string | null> => {
  try {
    return await stringStorage.get(DAILY_REWARD_KEY);
  } catch (error) {
    if (__DEV__) {
